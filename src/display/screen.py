...
class Screen:
    ...
    def display_user_manual(self) -> None:
        ...
        self._console.out(
            'f - Find path with current maze (start and end points will be generated if not done so already)')
        self._console.out('p - Generate random start and end points')
        self._console.out(
            'c - Clear maze colors and reset start and end points')
    ...
    def _get_display_dimensions(self) -> Dimensions:
        while True:
            try:
                item_validator = Validator(MatchesType(int()), InRange(
                    ValueRange(minimum=1, maximum=INF, inclusiveMin=True)))
                set_validator = Validator(HasLength(2))
                args = self._console.request(
                    'Window size?', set_validator, item_validator)
    ...
    def _get_cell_dimensions(self) -> Dimensions:
        while True:
            try:
                ...
                args = self._console.request(
                    'Cell size?', set_validator, item_validator)
